...
#else
static int pid = PID_NORMAL; // pid for general release
#endif
#else
...
static void usb_switch_to_cdrom(struct work_struct *w)
{
    ...
    /* prevent USB switching when the USB pid is PID_NORMAL */
...
static void usb_do_work(struct work_struct *w)
{
	...
	for (;;) {
		...
		if (flags == 0)
			break;
#ifdef CONFIG_USB_AUTO_INSTALL
		USB_PR("%s: ui->state=%d, flags=0x%lx\n", __func__, ui->state, flags);
#endif
		switch (ui->state) {
		case USB_STATE_IDLE:
					  ...
					  const char *buf, size_t size)
{
	...
	if (!strict_strtoul(buf, 10, &pid_index))
    {
        ...
        if(0 == rval)
        {
            USB_PR("Fixusb write OK! nv(%d)=%lx, rval=%d\n", nv_item, pid_index, rval);
        }
        else
        {
            USB_PR("Fixusb write failed! nv(%d)=%lx, rval=%d\n", nv_item, pid_index, rval);
        }
...
#endif  // endif of CONFIG_USB_AUTO_PID_ADAPTER
