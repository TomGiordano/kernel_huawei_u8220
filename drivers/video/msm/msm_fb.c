...
extern int fb_get_fota_mode(void);
#endif
					...
					enum led_brightness value)
{
    ...
    /*
	reboot and find the true backlight is not max value,
	but the setting is max value.
    */
...
#ifdef CONFIG_FB_DUMP_INFO
extern int mdp_get_dump_info(struct dump_mdp_info* dmdpi);
extern int mddi_get_dump_info(struct dump_mddi_info* dmddii);
extern int lcd_get_dump_info(struct dump_lcd_info* dlcdi);

static int msm_fb_get_dump_info(void __user* arg, 
	struct msm_fb_data_type* mfd)
{
	struct dump_all_info dalli;
	struct dump_mdp_info dmi;
	struct dump_mddi_info dmdi;
	struct dump_lcd_info dli;
	if(!mfd) return -EINVAL;
	
	if(!mfd->panel_power_on || !s_fb_dump_flag)
		return 0;
		
	if (copy_from_user(&dalli,
			arg,
			sizeof(struct dump_all_info)))
		return -EFAULT;

	//
	mdp_get_dump_info(&dmi);
	memcpy(&(dalli.dmdp), &dmi, sizeof(struct dump_mdp_info));

	mddi_get_dump_info(&dmdi);
	memcpy(&(dalli.dmddi), &dmdi, sizeof(struct dump_mddi_info));

	lcd_get_dump_info(&dli);
	memcpy(&(dalli.dlcd), &dli, sizeof(struct dump_lcd_info));
			...
			unsigned long arg)
{
	...
	switch (cmd) {
#ifdef CONFIG_FB_DUMP_INFO
	case MSMFB_GET_DUMP_INFO:
		ret = msm_fb_get_dump_info(argp, mfd);
		break;
#endif
