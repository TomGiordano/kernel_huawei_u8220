...
/* 
 ===================================================
 This region is used for defining the PANEL SEQUENCES
 NOTE:
 1. You can define a diffrent panel, but pls implement functions
 in the structure "ps_panel_seq_t";
 2. Panel Auto-adapted can implement in the function
 mddi_tc358723xbg_panel_detected. you can fix it by youself.
 ===================================================
 */

/*
 ===================================================
 The Common Part for ALL Panels
 */
...
typedef enum{
    ...
    LCD_PANEL_TIANMA,
    LCD_PANEL_MAX,
    LCD_PANEL_NONE = 0XFFFF,
}lcd_panel_type;
lcd_panel_type mddi_lcd_panel = LCD_PANEL_SAMSUNG;

const char* mddi_get_lcd_panel(void)
{
	char* panel;
	switch( mddi_lcd_panel ){
	case LCD_PANEL_SAMSUNG:
		panel =  "SAMSUNG"; break;
	case LCD_PANEL_QUNCHUANG:
		panel = "QUNCHUANG"; break;
	default:
		panel = "unknown"; break;
	}
	return panel;
}
EXPORT_SYMBOL(mddi_get_lcd_panel);
...
/*
 ===================================================
 The sequences of LMS350DF04 (SAMSUNG)
*/
...
#if 1
static inline void serigo_reg(uint8 reg)
{
	uint32 spid[2], ssiints, i;
	spid[0] = 0x00080074;
...
static inline void serigo_val(uint16 value)
{
	uint32 spid[2], ssiints, i;
	spid[0] = 0x00080076;
...
static uint32 serigo_read_reg(uint8 reg)
{
	uint32 spid[3], ssiints, ssirx, i;
	spid[0] = 0x00080074;
	spid[1] = 0x00010000 | reg;
	spid[2] = 0x00000079;
	mddi_queue_register_read(SSIINTS, &ssiints, TRUE, 0);
	...
	// read
	mddi_queue_register_read(SSIINTS, &ssiints, TRUE, 0);
	if((ssiints & 0x00001000) || (!(ssiints & 0x00000200)))
	   mddi_wait(1);
	if(!(ssiints & 0x00000200))
	   mddi_wait(1);
	
	mddi_queue_register_read(SSIRX, &ssirx, TRUE, 0);
	
	//mddi_queue_register_read(SSITX, &ssirx, TRUE, 0);
	
 	return ssirx;
}
#endif

static inline void s_reg_val_op(uint8 reg, uint16 val)
{
	if(0 == reg)
		mddi_wait(val);
	else
...
/* power on op */
static inline  void s_lms350df04_power_on(void)
{
	int i = 0;
	int count = 0;
...
/* power off  op*/ 
static inline void s_lms350df04_power_off(void)
{
	int i = 0;
...
/* display on  op*/ 
static inline void s_lms350df04_display_on(void)
{
	int i = 0;
...
/* display off  op*/ 
static inline void s_lms350df04_display_off(void)
{
	int i = 0;
...
/* standby in  op*/ 
static inline void s_lms350df04_standby_in(void)
{
	int i = 0;
...
/* standby out  op*/ 
static inline void s_lms350df04_standby_out(void)
{
	// state check
	//MDDI_MSG_ERR("Panel state (%d)\n", s_panel_state);
...
/*
static s_panel_seq_t s_lms350df04_panel = {
	.power_on = s_lms350df04_power_on,
	.power_off = s_lms350df04_power_off,
	.display_on = s_lms350df04_display_on,
	.display_off = s_lms350df04_display_off,
	.standby_in = s_lms350df04_standby_in,
	.standby_out = s_lms350df04_standby_out,
};
*/
/*
 ===================================================
 The sequences of ILI9481DS (BYD)
*/

/*********************fangchao注释**************
函数是为了实现spi对寄存器的命令操作，这是针对这款屏做的修改*/
static inline void LCD_ILI9481_CMD(uint8 reg) 
{ 
        uint32 spid; 
		
        spid = 0x00080000 | reg;    // CONT=0(CS deassert) CMD=0(Write) BYTE[1:0]=0(1byte) 

       mddi_queue_register_write(SSICTL,0x000000179,TRUE,0);         // SSICTL.SETACT=0         
       mddi_queue_register_write(SSITX,spid,TRUE,0);     

	   //start transfer
       mddi_queue_register_write(SSICTL,0x0000017a,TRUE,0);       // SSICTL.SETACT=1 

} 

/********************fangchao注释**************
函数是为了实现spi对寄存器的写操作，这是针对这款屏做的修改*/

static inline void LCD_ILI9481_INDEX(uint8 reg) 
{ 
    uint32 spid; 
        spid = 0x00000100 | reg;    // CONT=0(CS deassert) CMD=0(Write) BYTE[1:0]=0(1byte) 
        

        mddi_queue_register_write(SSICTL,0x000000179,TRUE,0);         // SSICTL.SETACT=0         
        mddi_queue_register_write(SSITX,spid,TRUE,0);         
        mddi_queue_register_write(SSICTL,0x00000017a,TRUE,0);        // SSICTL.SETACT=1 
}
static inline void Delayms(uint8 x) 
{
	mddi_wait(x);
}
...
typedef struct 
{
	void (*pfunc)(uint8 reg);
	uint8 param;
}s_pair_ili9481ds_on_seq;
/*********************fangchao注释**************
ic的初始化序列*/
s_pair_ili9481ds_on_seq ili9481ds_init_seq[] ={
    {LCD_ILI9481_CMD,0x01}, 
	{Delayms,120},  //200 ->20
	...
	{LCD_ILI9481_INDEX,0X07},  //0x07->0x02
	{LCD_ILI9481_INDEX,0X47}, 
	{LCD_ILI9481_INDEX,0X1B},  //0x1d->0x1f
	{Delayms,20},  //200 ->20

	{LCD_ILI9481_CMD,0XD1}, 
	{LCD_ILI9481_INDEX,0X00},  
	{LCD_ILI9481_INDEX,0X08},  
	{LCD_ILI9481_INDEX,0X19}, 
	...
	{LCD_ILI9481_INDEX,0x04}, //0x00->0x04
	...
	{LCD_ILI9481_INDEX ,0x83}, //0x00->0x03
	...
	{LCD_ILI9481_INDEX,0X70}, 
	{LCD_ILI9481_INDEX,0X00}, 
	{LCD_ILI9481_INDEX,0X09},  
	{LCD_ILI9481_INDEX,0X00}, 
	...
	{LCD_ILI9481_CMD,0XE4}, 
	{LCD_ILI9481_INDEX,0XA0}, 
	//{LCD_ILI9481_CMD,0x0c}, 	 
	//{LCD_ILI9481_INDEX,0x00}, 
...
s_pair_ili9481ds_on_seq ili9481ds_sleep_out_seq[] ={

    {LCD_ILI9481_CMD,0x11},
    {Delayms,150},  //50->150
};

s_pair_ili9481ds_on_seq ili9481ds_sleep_in_seq[] ={
...
/* power on op */
static inline void s_ili9481ds_power_on(void)
{
	int i = 0;
	int count = 0;
	printk(KERN_INFO"Panel state (%d)\n", s_panel_state);
	...
	switch(s_panel_state){
	case E_STATE_STANDBY:
        	count = sizeof(ili9481ds_sleep_in_seq)/sizeof(s_pair_ili9481ds_on_seq);
        	for(i = 0; i < count; i++)
        	{
        		ili9481ds_sleep_in_seq[i].pfunc(ili9481ds_sleep_in_seq[i].param);
        	}
	case E_STATE_POWEROFF:
        	count = sizeof(ili9481ds_init_seq)/sizeof(s_pair_ili9481ds_on_seq);
        	for(i = 0; i < count; i++)
        	{
        		ili9481ds_init_seq[i].pfunc(ili9481ds_init_seq[i].param);
        	}
		s_panel_state = E_STATE_POWERON;
		break;
	default:
...
/* power off  op*/ 
static inline void s_ili9481ds_power_off(void)
{
	int i = 0;
	int count = sizeof(s_poweroff_seq)/
		sizeof(s_pairs_seq_t);
...
/* display on  op*/ 
static inline void s_ili9481ds_display_on(void)
{
	int i = 0;
	int count = sizeof(s_displayon_seq)/
		sizeof(s_pairs_seq_t);
	...
	if((s_panel_state != E_STATE_POWERON) &&
    	...
    	count = sizeof(ili9481ds_sleep_out_seq)/sizeof(s_pair_ili9481ds_on_seq);
    	for(i = 0; i < count; i++)
    	{
    		ili9481ds_sleep_out_seq[i].pfunc(ili9481ds_sleep_out_seq[i].param);
    	}
	s_panel_state = E_STATE_DISPLAYON;
}

/* display off  op*/ 
static inline void s_ili9481ds_display_off(void)
{
	int i = 0;
	int count = sizeof(s_displayoff_seq)/
		sizeof(s_pairs_seq_t);
...
/* standby in  op*/ 
static inline void s_ili9481ds_standby_in(void)
{
	int i = 0;
	int count = sizeof(s_standby_in_seq)/
		sizeof(s_pairs_seq_t);
...
/* standby out  op*/ 
static inline void s_ili9481ds_standby_out(void)
{
	// state check
	//MDDI_MSG_ERR("Panel state (%d)\n", s_panel_state);
	if(s_panel_state != E_STATE_STANDBY){
	...
	// poweron
	s_ili9481ds_power_on();
	// displayon
	s_ili9481ds_display_on();
	
	s_panel_state = E_STATE_DISPLAYON;	
}
/*
static s_panel_seq_t s_ili9481ds_panel = {
	.power_on = s_ili9481ds_power_on,
	.power_off = s_ili9481ds_power_off,
	.display_on = s_ili9481ds_display_on,
	.display_off = s_ili9481ds_display_off,
	.standby_in = s_ili9481ds_standby_in,
	.standby_out = s_ili9481ds_standby_out,
};
*/

/* 
 ===================================================
 This region is used for defining funcs logics for  the MDDI TC358723XBG chip
 NOTE:
 You mustn't care about this partition unless you need to change 
 1, the refresh rate, you need to modify prim start sequence;
 2, the spi mode, you need to modify common init sequence.
 ===================================================
 */
...
s_seq_init_setup[]={
	...
	{GPIODATA,		0x00040000},	//	 # GPI .GPIODATA  # GPIO2(RESET_LCD_N) set to 0 , GPIO3(eDRAM_Power) set to 0
...
s_seq_init_setup_qunchuang[]={
	...
	{SSICTL,		0x00080078},//{SSICTL,			0x00000379},	//179
	{SSITIME,		0x00000100},	
	{CNT_DIS,		0x00000000},	
	{SSICTL,        0x0008007A},//{SSICTL,			0x0000037b},	
...
static void tc358723xbg_common_initial_setup(void)
{
    ...
    if(LCD_PANEL_SAMSUNG == mddi_lcd_panel)
    {
	for(i=0; i<count; i++)
    ...
    {
        count = sizeof(s_seq_init_setup_qunchuang) / sizeof(s_reg_val_pair_t);
    	for(i=0; i<count; i++)
    		s_reg_val_pair_op(s_seq_init_setup_qunchuang[i].reg, 
    			s_seq_init_setup_qunchuang[i].val);
	s_ili9481ds_power_on();
    }
...
static s_reg_val_pair_t
s_seq_prim_start_qunchuang[]={
	{BITMAP0,		0x01E00140},	// MDC.BITMAP1	); // Setup of PITCH size to Frame buffer1(VGA)
	{CLKENB,			0x000AF1EB},	// SYS.CLKENB  ); // DCLK supply 9.8MHz
	{PORT_ENB, 		0x00000001},	// LCD.PORT_ENB  ); // Synchronous port enable
	{PORT, 			0x00000004},	// LCD.PORT  ); // Polarity of DE is set to high active//04
	{CMN, 			0x00000000},	// LCD.CMN  ); // for rotate 180'
#ifdef CONFIG_HUAWEI_MDDI_LCD_24BIT // for BGR setting
	{PXL, 			0x0000023A},	// LCD.PXL  ); // 24bit and ACTMODE 2 set (1st frame black data output)
#else
...
static void tc358723xbg_prim_start(void)
{
    ...
    {
        count = sizeof(s_seq_prim_start_qunchuang) / sizeof(s_reg_val_pair_t);
    	for(i=0; i<count; i++)
    		s_reg_val_pair_op(s_seq_prim_start_qunchuang[i].reg, 
    			s_seq_prim_start_qunchuang[i].val);
	s_ili9481ds_display_on();
    }
	// primary display on
	//s_lms350df04_display_on();
...
{
    if (mddi_tc358723xbg_pdata && mddi_tc358723xbg_pdata->get_panel_type) 
    {
        mddi_lcd_panel = mddi_tc358723xbg_pdata->get_panel_type();
	}
    else
    {
        mddi_lcd_panel = LCD_PANEL_SAMSUNG;
    }
	return;
#if 0
	...
	if (!panel_detect) {
	   ...
	   /* not need here to change GPIODIR as input 
	   for input has been defined in init sequence ahead,
	   here config it as input cause below read GPIODATA error(very soon) --start--*/
#if 0
		read_value = read_client_reg(GPIODIR);
		read_value = read_value & (~(1 << 7));
		write_client_reg(GPIODIR, read_value, TRUE);
		mdelay(1);
#endif
	   /* not need here to change GPIODIR as input 
	   for input has been defined in init sequence ahead,
	   here config it as input cause below read GPIODATA error(very soon) --end--*/

		read_value = read_client_reg(GPIODATA);
		read_value = read_value & (1 << 7);
		
		if(read_value)
		{
			//printk( KERN_INFO "***********LCD PANEL = LCD_PANEL_BYD***********; \n");
			//return LCD_PANEL_BYD;
			 mddi_lcd_panel = LCD_PANEL_BYD;
			//panel_sel = &s_ili9481ds_panel;
		}
		else
		{
			//printk( KERN_INFO "***********LCD PANEL = LCD_PANEL_SAMSUNG***********; \n");
			//return LCD_PANEL_SAMSUNG;
			mddi_lcd_panel = LCD_PANEL_SAMSUNG;
			 //panel_sel = &s_lms350df04_panel;
		}
		panel_detect = 1;
	}
#endif
...
/*cancel  backlight_work and backlight_timer*/
#if 0
static boolean backlight_off = FALSE;
static struct work_struct backlight_work;
static struct hrtimer backlight_timer;
static enum hrtimer_restart backlight_timer_timer_cb(struct hrtimer *timer)
{
	schedule_work(&backlight_work);
	return HRTIMER_NORESTART;
}
static void backlight_work_cb(struct work_struct *work)
{
	if(backlight_off)
	{
			int counter = 12;
	...
	else
	{
        ...
        mddi_tc358723xbg_prim_init();
		hrtimer_start(&backlight_timer,
			ktime_set(100 / 1000, (100 % 1000) * 1000000),
			HRTIMER_MODE_REL);
	}
}
#endif
static int mddi_tc358723xbg_lcd_on(struct platform_device *pdev)
{
    ...
    if (lcd_off == FALSE)
    {
        ...
        /*cancel  backlight_work and backlight_timer*/
	/*backlight_off = FALSE;	
	INIT_WORK(&backlight_work, backlight_work_cb);
	hrtimer_init(&backlight_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	backlight_timer.function = backlight_timer_timer_cb;	
	hrtimer_start(&backlight_timer,
		ktime_set(2000 / 1000, (2000 % 1000) * 1000000),
		HRTIMER_MODE_REL);	
	backlight_off = TRUE;*/
	/*close LCD backlight*/
	mddi_tc358723xbg_lcd_off(NULL);
	...
	mddi_tc358723xbg_panel_detect();
/*	if(mddi_lcd_panel == LCD_PANEL_BYD){
		mfd->panel_info.clk_max = 120000000;
		mfd->panel_info.clk_min = 120000000;
		mfd->panel_info.clk_rate = 120000000;
		mfd->panel_info.lcd.refx100 = 7200;
	}*/
...
static void mddi_tc358723xbg_reset(void)
{
	...
	//modify reset timing according to BYD LCD SPEC
	mdelay(10);
...
#ifdef CONFIG_FB_DUMP_INFO
int lcd_get_dump_info(struct dump_lcd_info* dlcdi)
{
	if(dlcdi == NULL)
		return -1;
	dlcdi->lcdtype = (unsigned int)mddi_lcd_panel;
	dlcdi->bstatus = (unsigned int)tc358723xbg_state;
	dlcdi->pstatus = (unsigned int)s_panel_state;
	
	dlcdi->dpset0 = read_client_reg(DPSET0);
	dlcdi->dpset1 = read_client_reg(DPSET1);
	dlcdi->dpsus = read_client_reg(DPSUS);
	dlcdi->dprun = read_client_reg(DPRUN);
	dlcdi->sysckena = read_client_reg(SYSCKENA);
	dlcdi->clkenb = read_client_reg(CLKENB);
	dlcdi->gpiodata = read_client_reg(GPIODATA);
	dlcdi->gpiodir = read_client_reg(GPIODIR);
	dlcdi->gpiosel = read_client_reg(GPIOSEL);
	dlcdi->intmsk = read_client_reg(INTMSK);
	dlcdi->wakeup = read_client_reg(WAKEUP);
	dlcdi->intmask = read_client_reg(INTMASK);
	dlcdi->drampwr = read_client_reg(DRAMPWR);
	dlcdi->ssictl = read_client_reg(SSICTL);
	dlcdi->ssitime = read_client_reg(SSITIME);
	dlcdi->cnt_dis = read_client_reg(CNT_DIS);
	
	dlcdi->bitmap0 = read_client_reg(BITMAP0);
	dlcdi->port_enb = read_client_reg(PORT_ENB);
	dlcdi->port = read_client_reg(PORT);
	dlcdi->cmn = read_client_reg(CMN);
	dlcdi->pxl = read_client_reg(PXL);
	dlcdi->mplfbuf = read_client_reg(MPLFBUF);
	dlcdi->hde_left = read_client_reg(HDE_LEFT);
	dlcdi->vde_top = read_client_reg(VDE_TOP);
	dlcdi->hcycle = read_client_reg(HCYCLE);
	dlcdi->hsw = read_client_reg(HSW);
	dlcdi->hde_start = read_client_reg(HDE_START);
	dlcdi->hde_size = read_client_reg(HDE_SIZE);
	dlcdi->vcycle = read_client_reg(VCYCLE);
	dlcdi->vsw = read_client_reg(VSW);
	dlcdi->vde_start = read_client_reg(VDE_START);
	dlcdi->vde_size = read_client_reg(VDE_SIZE);
	dlcdi->regenb = read_client_reg(REGENB);
	dlcdi->start = read_client_reg(START);
	
	dlcdi->lcd = serigo_read_reg(0x0c);
	return 0;
}
#endif
