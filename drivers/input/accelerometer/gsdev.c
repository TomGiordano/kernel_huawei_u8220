...
#define TRACE_GS 0
#if TRACE_GS
#define GS(x...) printk(KERN_INFO "[GSdev] " x)
#else
#define GS(x...) do {} while (0)
#endif
...
int gs_adi_data_to_compass(int accel_data [3])
{
    ...
    GS("%s,accel_data[0]=%d, accel_data[1]=%d, accel_data[2]=%d \n\n",__FUNCTION__,accel_data[1],accel_data[0],accel_data[2]);
    return 0;
...
{	
	...
	ret =  reg_write(gs,GS_ADI_REG_BW,0x0a);    /* Rate: 100Hz, IDD: 130uA */
     ...
     ret = reg_write(gs,GS_ADI_REG_DATA_FORMAT,0x0b);  /* 13bit,16g */
     if ( ret <0 )
			 goto err_detect_failed;
     ...
     ret = reg_write(gs,GS_ADI_REG_TAP_AXES,0x08);	
	 if ( ret <0 )
		goto err_detect_failed;
