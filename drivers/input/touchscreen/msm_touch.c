...
#define TS_KEY_DEBOUNCE_TIMER_MS 50
...
static RECORD_EXTRA_KEYCODE  record_extra_keycode = {KEY_RESERVED, TRUE, TRUE};
...
static void update_pen_and_key_state(struct ts *ts, int x, int y, int pressure)
{
    ...
    if(pressure)  /*press*/
    {
        ...
        {
            /* 如果记录键值还没有释放，则返回 */
            if (FALSE == record_extra_keycode.bRelease 
                && KEY_RESERVED != record_extra_keycode.record_extra_key)
            {
                return;
            }
...
static int __devinit ts_probe(struct platform_device *pdev)
{
    ...
    if(ts->use_touch_key)
    {
        input_set_abs_params(input_dev, ABS_X, 60, 960, 0, 0);
        input_set_abs_params(input_dev, ABS_Y, 50, 920, 0, 0);
    }
    else
    {
