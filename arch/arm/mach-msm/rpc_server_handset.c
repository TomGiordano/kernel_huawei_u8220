...
struct wake_lock hs_init_wake_lock;
...
report_headset_switch(struct input_dev *dev, int key, int value)
{
    ...
    #ifdef CONFIG_HUAWEI_APPS
	wake_lock_timeout(&hs_init_wake_lock, 2 * HZ);
	#endif /*CONFIG_HUAWEI_APPS*/
...
static int __devinit hs_rpc_init(void)
{
	...
	if (machine_is_msm7x27_surf() || machine_is_msm7x27_ffa() ||
		...
		machine_is_msm7x25_u8110()|| machine_is_msm7x25_u8120() ||
		machine_is_msm7x25_u7610()|| machine_is_msm7x25_c8600() ) {
    ...
    #ifdef CONFIG_HUAWEI_APPS
	wake_lock_init(&hs_init_wake_lock, WAKE_LOCK_SUSPEND, "hs_wakelock_init");
    #endif /*CONFIG_HUAWEI_APPS*/
...
static void __devexit hs_rpc_deinit(void)
{
    ...
    #ifdef CONFIG_HUAWEI_APPS
	{
        wake_lock_destroy(&hs_init_wake_lock);
	#endif /*CONFIG_HUAWEI_APPS*/		
		msm_rpc_unregister_client(rpc_client);
	#ifdef CONFIG_HUAWEI_APPS	
    }
